        ...
        workbook.SheetNames.forEach((sheetName) => {
            ...
            for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
                ...
                const rowObj = {
                    ...
                    fortran_file: columns.fortran_file !== -1 ? cleanValue(row[columns.fortran_file]) : '',
                    python_script: columns.python_script !== -1 ? cleanValue(row[columns.python_script]) : '',
                    p: rawP,  //  STORE AS-IS (P2, P1, etc.) - matches template folder structure
                    l: rawL   //  STORE AS-IS (L1, L2, etc.) - matches template folder structure
                };
