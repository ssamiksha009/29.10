...
app.post('/api/create-protocol-folders', (req, res) => {
    ...
    try {
        ...
        if (!fs.existsSync(projectPath)) {
            fs.mkdirSync(projectPath, { recursive: true });
            console.log(` Created base folder: ${projectPath}`);
        }
...
if (!fs.existsSync(logsPath)) {
    ...
    console.log(`Created logs directory: ${logsPath}`);
        ...
        //  FOR CDTIRE: Read subfolders from templates/CDTire
        let subfolders;
        
        if (protocol === 'CDTire') {
            console.log(` Processing CDTire protocol with nested subfolders...`);
            
            // Get all P_L subfolders from templates/CDTire
            subfolders = fs.readdirSync(templatePath).filter(item => {
                const itemPath = path.join(templatePath, item);
                return fs.statSync(itemPath).isDirectory() && /^P\d+_L\d+$/.test(item);
            });
            
            if (subfolders.length === 0) {
                throw new Error(`No P_L subfolders found in CDTire template: ${templatePath}`);
            }
            
            console.log(` Found ${subfolders.length} CDTire subfolders: ${subfolders.join(', ')}`);
        } else {
            // For other protocols, use standard logic
            subfolders = fs.readdirSync(templatePath).filter(item =>
            ...
            console.log(` Found ${subfolders.length} subfolders for ${protocol}`);
        }

        // Copy all template subfolders to the project folder
        subfolders.forEach(subfolder => {
            ...
            copyFolderRecursiveSync(sourceSubfolder, destSubfolder);
            
            console.log(`    Copied: ${subfolder} → ${projectPath}`);
        });
        ...
        if (fs.existsSync(centralParametersPath)) {
            console.log(` Copying parameters.inc to subfolders...`);
            ...
            subfolders.forEach(subfolder => {
                ...
                try {
                    ...
                    console.warn(`     Failed to copy parameters.inc to ${subfolder}:`, copyErr.message);
                }
            });
        } else {
            console.warn(`  parameters.inc not found at: ${centralParametersPath}`);
        }
        ...
        if (fs.existsSync(tempDir)) {
            ...
            if (meshFiles.length > 0) {
                ...
                console.log(` Copying mesh file: ${meshFile}`);

                subfolders.forEach(subfolder => {
                    const destMeshPath = path.join(projectPath, subfolder, meshFile);
                    try {
                        fs.copyFileSync(sourceMeshPath, destMeshPath);
                        console.log(`    Copied mesh to: ${subfolder}`);
                    } catch (copyErr) {
                        console.error(`    Error copying mesh to ${subfolder}:`, copyErr.message);
                    }
                });
                ...
                try {
                    ...
                    console.log(` Cleaned up temporary mesh file`);
                } catch (cleanupErr) {
                    console.error('  Error cleaning up temporary mesh file:', cleanupErr.message);
                }
            }
        ...
        try {
            if (fs.existsSync(centralParametersPath)) {
                ...
                console.log(` Cleaned up central parameters.inc file`);
            }
        } catch (cleanupErr) {
            console.error('  Error cleaning up central parameters.inc file:', cleanupErr.message);
        }
...
app.post('/api/resolve-job-dependencies', deduplicateRequests, async (req, res) => {
    ...
    // ✅ CRITICAL: No code after try/catch that could send response
 ...
 * Workflow Cases:
 * 1. deflection.py → generates 3 .f files → store in templates/CDTire
...
function executeAbaqusJob(folderPath, jobName, oldJobName, folderName = '', protocol = '', rowData = {}) {
    return new Promise((resolve, reject) => {
        // ✅ Extract project info from folder path
        ...
        const protocolPart = parts[parts.length - 1];
        const projectName = parts.slice(0, -1).join('_');
        
        console.log(`\n🔍 Extracted Project Info:`);
        console.log(`   Project Name: ${projectName}`);
        console.log(`   Protocol: ${protocol || protocolPart}`);
        console.log(`   Folder Path: ${folderPath}`);
        
        // ✅ Get logger (will be created if doesn't exist)
        const logger = getProjectLogger(projectName, protocol || protocolPart);
        
        if (!logger) {
            console.warn('⚠️ Could not create logger for project: ' + projectName);
        }
        
        const startTime = Date.now();
        
        if (logger) {
            logger.separator('ABAQUS JOB EXECUTION');
            logger.info(`Folder: ${folderPath}`);
            logger.info(`Job: ${jobName}`);
            logger.info(`Old Job: ${oldJobName || 'None'}`);
            logger.info(`Folder Name: ${folderName}`);
            logger.info(`Protocol: ${protocol}`);
            if (protocol.toLowerCase() === 'cdtire') {
                logger.info(`Fortran File: ${rowData.fortran_file || 'None'}`);
                logger.info(`Python Script: ${rowData.python_script || 'None'}`);
            }
            logger.separator();
        }
        
        console.log('\n' + '='.repeat(80));
        console.log('⚙️ EXECUTING ABAQUS JOB');
        console.log('='.repeat(80));
        ...
        // Determine which .inp file to use
        let inpPath = path.join(folderPath, jobName + '.inp');
        if (!fs.existsSync(inpPath) && oldJobName) {
            if (logger) logger.warn(`${jobName}.inp not found, trying old job: ${oldJobName}.inp`);
            console.log('  ' + jobName + '.inp not found, trying old job: ' + oldJobName + '.inp');
            inpPath = path.join(folderPath, oldJobName + '.inp');
            if (!fs.existsSync(inpPath)) {
                const errMsg = 'Neither ' + jobName + '.inp nor ' + oldJobName + '.inp found in ' + folderPath;
        ...
        if (!fs.existsSync(inpPath)) {
            const errMsg = 'Input file not found: ' + inpPath;
            if (logger) logger.error(errMsg);
            return reject(new Error(errMsg));
        }

        if (logger) logger.info(`Using input file: ${inpPath}`);
        console.log(' Using input file: ' + inpPath + '\n');

        // Run Abaqus Job
        const abaqusCmd = 'call abaqus job=' + jobName + ' input=' + jobName + '.inp interactive';
        if (logger) logger.info(`Abaqus command: ${abaqusCmd}`);
        console.log('  Abaqus command: ' + abaqusCmd);

        const abaqusProcess = spawn('cmd.exe', ['/c', abaqusCmd], {
            cwd: folderPath,
        ...
        abaqusProcess.stdout.on('data', (data) => {
            ...
            abaqusOutput += output;
            if (logger) logger.abaqusOutput(output);
            console.log(' Abaqus: ' + output.trim());
            ...
            broadcastSse('run-status', {
                ...
                status: 'running',
                stage: 'abaqus',
                message: 'Running Abaqus simulation...'
            });
        });

        abaqusProcess.stderr.on('data', (data) => {
            const errOutput = data.toString();
            if (logger) logger.error(`Abaqus stderr: ${errOutput}`);
            console.error('  Abaqus stderr: ' + errOutput);
        });
        ...
        abaqusProcess.on('close', async (code) => {
            ...
            if (logger) {
                logger.info(`Abaqus job completed with exit code: ${code}`);
                logger.info(`Duration: ${(duration / 1000).toFixed(2)}s`);
            }
            console.log('\n Abaqus job completed with exit code: ' + code);
            
            if (code !== 0) {
                const errorMsg = 'Abaqus job failed with exit code ' + code;
                if (logger) {
                    logger.error(errorMsg);
                    if (abaqusOutput.includes('***ERROR')) {
                        logger.error('Abaqus encountered errors. Check .msg and .dat files for details.');
                    }
                    logger.jobEnd(jobName, false, duration);
                }
                console.error(' ' + errorMsg);
                return reject(new Error(errorMsg));
            }

            if (logger) logger.success('Abaqus job executed successfully');
            console.log(' Abaqus job executed successfully\n');

            // ONLY run Fortran/Python workflow for CDTire protocol
            if (protocol.toLowerCase() === 'cdtire') {
                try {
                    const fortranFile = rowData.fortran_file || '';
                    const pythonScript = rowData.python_script || '';

                    if (logger) {
                        logger.separator('CDTIRE POST-PROCESSING');
                        logger.info(`Fortran File: ${fortranFile || 'None'}`);
                        logger.info(`Python Script: ${pythonScript || 'None'}`);
                    }

                    // Run Fortran if specified
                    if (fortranFile && fortranFile.trim() !== '' && fortranFile !== '-') {
                        if (logger) logger.info(`Running Fortran file: ${fortranFile}`);
                        await runFortranFile(folderPath, fortranFile, jobName, folderName, logger);
                    }

                    // Run Python if specified
                    if (pythonScript && pythonScript.trim() !== '' && pythonScript !== '-') {
                        if (logger) logger.info(`Running Python script: ${pythonScript}`);
                        await runPythonScript(folderPath, pythonScript, jobName, folderName, logger);
                    }

                    if (logger) logger.jobEnd(jobName, true, duration);
                    resolve();
                } catch (postJobError) {
                    if (logger) {
                        logger.error('Error in CDTire post-processing', postJobError);
                        logger.jobEnd(jobName, false, duration);
                    }
                    console.error(' Error in CDTire post-processing:', postJobError);
                    reject(postJobError);
                }
            } else {
                if (logger) {
                    logger.info('Standard protocol workflow complete (no post-processing)');
                    logger.jobEnd(jobName, true, duration);
                }
                console.log(' Standard protocol workflow complete (no post-processing)\n');
                resolve();
            }
        });

        abaqusProcess.on('error', (error) => {
            if (logger) logger.error('Failed to start Abaqus process', error);
            console.error(' Failed to start Abaqus process:', error);
            reject(new Error('Failed to start Abaqus: ' + error.message));
        });
    });
}
 ...
 * Fetches Python script from templates/CDTire folder
 * @param {string} folderPath - Path to the job folder
 * @param {string} pythonFileName - Name of the Python file (.py)
...
async function runPythonScript(folderPath, pythonFileName, jobName, folderName, logger = null) {
    return new Promise((resolve, reject) => {
        if (logger) {
            logger.separator('PYTHON SCRIPT EXECUTION');
            logger.info(`Python Script: ${pythonFileName}`);
            logger.info(`Job: ${jobName}`);
            logger.info(`Folder: ${folderName}`);
        }
        
        console.log('\n Running Python script: ' + pythonFileName);
        
        // Source: templates/CDTire/<python_file>
        const templatePythonPath = path.join(__dirname, 'templates', 'CDTire', pythonFileName);
        
        // Destination: job folder
        const localPythonPath = path.join(folderPath, pythonFileName);

        // Check if Python file exists in templates
        if (!fs.existsSync(templatePythonPath)) {
            const error = 'Python script not found in templates: ' + templatePythonPath;
            console.error(' ' + error);
            if (logger) logger.error(error);
            return reject(new Error(error));
        }

        console.log(' Found Python script in templates: ' + templatePythonPath);
        if (logger) logger.info('Found Python script in templates: ' + templatePythonPath);

        // Copy Python file to job folder
        try {
            fs.copyFileSync(templatePythonPath, localPythonPath);
            console.log(' Copied Python script to: ' + localPythonPath);
            if (logger) logger.info('Copied Python script to: ' + localPythonPath);
        } catch (copyErr) {
            console.error(' Failed to copy Python script:', copyErr);
            if (logger) logger.error('Failed to copy Python script', copyErr);
            return reject(copyErr);
        }

        // Run Python script using Abaqus Python
        // Command: abaqus python <python_file>
        const pythonCmd = 'abaqus python ' + pythonFileName;
        
        console.log('  Python command: ' + pythonCmd);
        if (logger) logger.info('Python command: ' + pythonCmd);

        const pythonProcess = spawn('cmd.exe', ['/c', pythonCmd], {
            cwd: folderPath,
            stdio: ['ignore', 'pipe', 'pipe']
        });
        ...
        pythonProcess.stdout.on('data', (data) => {
            ...
            broadcastSse('run-status', {
                ...
                status: 'running',
                stage: 'python',
                message: 'Running Python script: ' + pythonFileName
            });
        });
        ...
        pythonProcess.stderr.on('data', (data) => {
            ...
            if (logger) logger.error('Python stderr: ' + errOutput);
        });
        ...
        pythonProcess.on('close', (code) => {
            ...
            // ADD LOGGER HERE
            if (logger) logger.info('Python process completed with exit code: ' + code);
            
            if (code !== 0) {
                const error = 'Python execution failed with exit code ' + code;
                console.error(' ' + error);
                if (logger) logger.error(error);
                return reject(new Error(error));
            }
            ...
            if (logger) logger.success('Python script executed successfully');
            resolve();
        });

        pythonProcess.on('error', (error) => {
            console.error(' Failed to start Python process:', error);
            if (logger) logger.error('Failed to start Python process', error);
